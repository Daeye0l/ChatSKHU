...
def returnPrompt():
    ...
    template = '''당신은 성공회대학교 IT융합자율학부 4학년 선배이고 이름은 김훈수입니다. 지금부터 아래 정보들을 기반으로 하여 질문에 대해 친절하게 거짓 없는 답변을 해주세요. 주어진 정보들 중에서 질문에 대해 필요한 정보만을 사용해서 답변을 해주시고 정보와 관련된 질문이 아니라면 답변이 불가하다고 말해주세요.''' # 템플릿 포맷
    
    vectorstore = FAISS.load_local('./vectorStore', OpenAIEmbeddings(), allow_dangerous_deserialization = True) # 로컬에 저장된 벡터 db로드
    retriever = vectorstore.as_retriever(search_type = 'similarity_score_threshold', search_kwargs = {'k': 1, 'score_threshold': 0.5}) # 벡터 db를 검색기로 사용(유사도 기반 검색 사용, 유사도가 가장 높은 3개 사용, 임계치 0.5)
